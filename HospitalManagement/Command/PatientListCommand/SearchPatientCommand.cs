    ...
    class SearchPatientCommand : ICommand
    {
        ...
        public void Execute(object parameter)
        {
            ...
            else
            {
                ...
                {
                    patientViewModel.Patients = SelectableItem<BENHNHAN>.GetSelectableItems(
                                                                                            DataProvider.Ins.DB.BENHNHANs.Where(
                                                                                                p => p.CMND_CCCD.Contains(patientViewModel.SearchBox)).ToList());
                }
                else if (RemoveSignString == "ho")
                {
                    //Patients = DataProvider.Ins.DB.BENHNHANs.Where(p => p.HO.Contains(searchBox)).ToList();
                    patientViewModel.Patients = SelectableItem<BENHNHAN>.GetSelectableItems(
                    DataProvider.Ins.DB.BENHNHANs.Where(delegate (BENHNHAN bn)
                    {
